...
def make_param_groups(model, weight_decay):
    decay, no_decay, lora_p = [], [], []
    for name, p in model.named_parameters():
        if not p.requires_grad: continue
        if "lora_" in name.lower():       lora_p.append(p)
        elif name.endswith(".bias") or "norm" in name.lower(): no_decay.append(p)
        else:                             decay.append(p)
    groups = []
    if decay:    groups.append({"params": decay,    "weight_decay": weight_decay})
    if no_decay: groups.append({"params": no_decay, "weight_decay": 0.0})
    if lora_p:   groups.append({"params": lora_p,   "weight_decay": 0.0})
    return groups
...
def validate(model, dataloader, criterion, device):
    ...
    # macro AUC across all 8 classes
    try:
...
def main():
    ...
    img_dir_test  = f"{DATA_DIR}/ODIR-5K/testing"
    ...
    print(f"\n{'='*60}")
    print(f"STARTING TRAINING — RETFound LoRA ODIR Multi-Label")
    print(f"{'='*60}")
    ...
    # plots
    plot_save_dir = "./plots/retfound-mixed"

...
checkpoint_path = f"{SRC_DIR}/best_models/best_urfound_nonlora_model.pth"
...
test_loss, test_acc, precision, recall, f1, qwk, per_class_auc, macro_auc, weighted_auc, per_class_recall, per_class_precision, y_true, y_probs = test_urfound(
    model=model,
...
results_dir = os.path.join(BASE_DIR, "results", "urfound")

os.makedirs(results_dir, exist_ok=True)

results_path = os.path.join(results_dir, "urfound_nonlora_test_results.json")
...
json_to_csv(results_path, "results/urfound", "urfound_nonlora_test_results")

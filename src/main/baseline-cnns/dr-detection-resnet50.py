...
def train_one_epoch(model, dataloader, criterion, optimizer, device, scaler):
    ...
    for step, batch in enumerate(dataloader):
        # DR dataset can return (images, labels) or (images, labels, sources)
...
def validate(model, dataloader, criterion, device):
    ...
    return val_loss, val_acc, bal_acc, macro_f1, qwk, macro_auc, report
...
def main():
    ...
    train_csv_paths = {
        "DEEPDRID": f"{root_dirs['DEEPDRID']}/regular_fundus_images/regular-fundus-training/regular-fundus-training.csv",
        "EYEPACS":  f"{root_dirs['EYEPACS']}/all_labels.csv",
        "DDR":      f"{root_dirs['DDR']}/DR_grading.csv",
    }
    val_csv_paths = {
        "DEEPDRID": f"{root_dirs['DEEPDRID']}/regular_fundus_images/regular-fundus-validation/regular-fundus-validation.csv",
        "EYEPACS":  f"{root_dirs['EYEPACS']}/all_labels.csv",
        "DDR":      f"{root_dirs['DDR']}/DR_grading.csv",
    }
    ...
    val_dataset.load_labels_from_csv(val_csv_paths)
    train_dataset.prune_unlabeled()
    val_dataset.prune_unlabeled()
    ...
    print(f"\n{'=' * 60}")
    print(f"STARTING TRAINING — ResNet50 DR Grading")
    print(f"{'=' * 60}")
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        val_loss, val_acc, val_bal_acc, val_macro_f1, val_qwk, val_macro_auc, report = validate(
        ...
        history_qwk.append(val_qwk)

        print(f"val_acc={val_acc:.2f}% | bal_acc={val_bal_acc:.2f}% | "
              f"macro_f1={val_macro_f1:.2f}% | qwk={val_qwk:.4f} | "
              f"macro_auc={val_macro_auc:.2f}%")
        print(report)
    ...
    plot_save_dir = "./plots/resnet50-dr"
    ...
    save_metric_plot(history_epochs, history_qwk,       "QWK",                 plot_save_dir)
    ...
    with open(f"{SRC_DIR}/best_models/resnet50-dr-benchmark.json", "w") as f:
        json.dump(benchmark, f, indent=4)

    plot_all_benchmark(
        source=benchmark,
        output_dir=f"{SRC_DIR}/plots/baseline-plots/resnet50-dr-benchmark-plots",
        skip=1,
        model_name="ResNet50 DR Grading",
    )

    save_path = f"{SRC_DIR}/best_models/best_resnet50_dr_model.pth"
    torch.save({
